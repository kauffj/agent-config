...
// Shared harness-support topology. Paths are relative to HOME; targets are relative
...
export const HARNESS_LINK_LAYOUT = [
  ...
  },
  {
    ...
    compatibleHomePaths: ['.claude/codex/hooks.json'],
  },
  {
...
export function resolveHarnessLinks(repo, home) {
  return HARNESS_LINK_LAYOUT.map((entry) => ({
    ...
    compatibleLinkTargets: [
      ...(entry.compatibleLinkTargets || []),
      ...(entry.compatibleHomePaths || []).map((path) => resolve(home, path)),
    ],
  }));
}
