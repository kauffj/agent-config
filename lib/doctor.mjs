#!/usr/bin/env node
// Repo integrity checker for the shared agent configuration.
//
// Resolves every cross-reference in the config — file paths embedded in
// skills/agents/commands/hooks/instructions, settings.json `env` values, and
// harness support links — against what is actually on disk, and reports
// anything dangling.
...
// Canonical (symlink-resolved) repo root, so discovery checks below
...
const SCAN_DIRS = ['skills', 'agents', 'commands', 'hooks', 'codex', 'instructions'];
...
//   1. A path below ~/.claude or the neutral ~/.config/agent-config alias
...
function checkRefsInText(text, source) {
  const rels = new Set();
  const homeRe = /(?:\$HOME|\$\{HOME\}|~)\/(?:\.claude|\.config\/agent-config)\/([A-Za-z0-9._/-]+)/g;
...
function resolveEnvPath(val) {
  const v = val.trim();
  let m;
  if ((m = v.match(/^(?:\$HOME|\$\{HOME\}|~)\/(?:\.claude|\.config\/agent-config)\/(.+)$/))) {
    return join(REPO, m[1]);
  }
...
function checkSettingsEnv() {
  ...
  for (const [key, val] of Object.entries(settings.env ?? {})) {
    ...
    const under = val.trim().match(/^(?:\$HOME|\$\{HOME\}|~)\/(?:\.claude|\.config\/agent-config)\/(.+)$/);
...
function checkCounts() {
  ...
  for (const { dir, count } of COUNTED) {
    ...
    try {
      const prefix = `${dir}/`;
      const tracked = new Set(execFileSync('git', ['ls-files', '-z', '--', dir], {
        cwd: REPO,
        encoding: 'utf8',
        stdio: ['ignore', 'pipe', 'ignore'],
      }).split('\0').filter(Boolean).map((path) => path.slice(prefix.length).split('/')[0]));
      shipped = entries.filter((entry) => tracked.has(entry.name));
    } catch {
      // A copied/non-Git installation has no tracked-file boundary; the files
      // on disk are the only inventory available there.
    }
    const actual = count(shipped);
...
function checkSupportLinks() {
  for (const spec of resolveHarnessLinks(REPO, HOME)) {
    const label = `~/${spec.homePath}`;
    const stat = lstatOrNull(spec.linkPath);
    if (stat === null) {
      warn(`${label}: support link is missing — run bin/agent-config-install`);
      continue;
    }
    if (!stat.isSymbolicLink()) {
      warn(`${label}: expected a symbolic link to ${quoted(spec.targetPath)}`);
      continue;
    }
    const linkTarget = readlinkSync(spec.linkPath);
    const destination = resolve(dirname(spec.linkPath), linkTarget);
    let resolvedTarget = null;
    try { resolvedTarget = realpathSync(spec.linkPath); }
    catch { /* A dangling compatibility link is not correct. */ }
    if (resolvedTarget === null) {
      warn(`${label}: is dangling (target ${quoted(destination)})`);
      continue;
    }
    const targetStat = statSync(spec.linkPath);
    const correctType = spec.targetType === 'directory'
      ? targetStat.isDirectory()
      : targetStat.isFile();
    if (!correctType) {
      warn(`${label}: target is not a ${spec.targetType} (${quoted(destination)})`);
      continue;
    }
    if (classifyHarnessLinkTarget(spec, linkTarget, resolvedTarget) !== 'correct') {
      warn(`${label}: resolves to ${quoted(destination)}, not ${quoted(spec.targetPath)}`);
    }
  }
}
...
// autoMode.environment is the classifier's picture of this machine, and it is
// USER-scope only (project settings are ignored by design). /auto-mode-setup
// writes it from whichever repo it runs in and never refreshes it, so for two
// months it described only kauffj/education and named freestate.party a
// foreign "sensitive remote target", blocking routine fsp-app work. It cannot
// live in this public repo, so the authored copy lives in the private
// server-config repo; this check makes drift between the two loud.
const AUTOMODE_SOURCE = join(HOME, 'projects/server-config/agent-automode.json');
function checkAutoModeEnvironment() {
  if (!existsSync(AUTOMODE_SOURCE) || !existsSync(settingsPath)) return;
  let want, have;
  try {
    want = JSON.parse(readFileSync(AUTOMODE_SOURCE, 'utf8'))?.autoMode?.environment;
    have = JSON.parse(readFileSync(settingsPath, 'utf8'))?.autoMode?.environment;
  } catch { return; }
  if (JSON.stringify(want) === JSON.stringify(have)) return;
  warn(`settings.json autoMode.environment differs from ${AUTOMODE_SOURCE} (the authored, all-projects copy; /auto-mode-setup overwrites it with one repo's view). Restore: node -e 'const f=require("fs"),s="${settingsPath}",e=JSON.parse(f.readFileSync("${AUTOMODE_SOURCE}")).autoMode.environment,d=JSON.parse(f.readFileSync(s));d.autoMode={...d.autoMode,environment:e};f.writeFileSync(s,JSON.stringify(d,null,2)+"\\n")' — or, if the live one is the intended update, copy it into the source file and commit server-config.`);
}

// --- run --------------------------------------------------------------------
// A gitignored SOURCE is not this repo's either: skills/synced/ is the CLI's
// claude.ai skill-sync cache, whose SKILL.md files cite paths that only exist
// inside their own bundle. Same rule as classifyMissing, applied to the scan
// side, so an ignored tree cannot fail the commit gate for a file we don't ship.
for (const d of SCAN_DIRS) {
  const dir = join(REPO, d);
  if (!existsSync(dir)) continue;
  const files = walk(dir).filter((f) => SCAN_EXTS.has(f.slice(f.lastIndexOf('.'))));
  const ignored = ignoredSet(files.map(relToRepo));
  for (const f of files) {
    if (ignored.has(relToRepo(f))) continue;
