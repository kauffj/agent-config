...
// skills/agents/commands/hooks, settings.json `env` values, and the ~/.claude
// symlink — against what is actually on disk, and reports anything dangling.
...
const SCAN_DIRS = ['skills', 'agents', 'commands', 'hooks'];
...
function checkRefsInText(text, source) {
  ...
  const relRe = /\b(?:agents|lib|hooks|skills|commands)\/[A-Za-z0-9._/-]*\.(?:md|mjs|js|sh|json|ts)\b/g;
  let m;
  while ((m = homeRe.exec(text))) rels.add(stripTrail(m[1]));
  while ((m = relRe.exec(text))) rels.add(stripTrail(m[0]));
  for (const rel of rels) {
    if (!existsSync(join(REPO, rel))) err(`${source}: references missing file → ${rel}`);
  }
}
...
const isUnderConfig = (val) => /^(?:\$HOME|\$\{HOME\}|~)\/\.claude\//.test(val.trim());
...
function checkSettingsEnv() {
  ...
  for (const [key, val] of Object.entries(settings.env ?? {})) {
    ...
    // Paths under the config dir MUST exist (the config ships them). Paths
    // elsewhere are machine-specific (e.g. a sibling repo) — warn, don't fail.
    if (isUnderConfig(val)) err(`settings.json env.${key}: path does not resolve → ${val}`);
