...
test('ignored runtime profiles refresh when the repository is renamed', (t) => {
  const root = fixture(t);
  const renamed = `${root}-renamed`;
  t.after(() => rmSync(renamed, { recursive: true, force: true }));
  assert.equal(JSON.parse(project(root, 'load').stdout).repoName, basename(root));

  renameSync(root, renamed);

  const profile = JSON.parse(project(renamed, 'load').stdout);
  const cached = JSON.parse(readFileSync(join(renamed, '.workspaces', 'project.json'), 'utf8'));
  assert.equal(profile.repoName, basename(renamed));
  assert.equal(cached.repoName, basename(renamed));
});
