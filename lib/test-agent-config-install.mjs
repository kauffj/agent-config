...
import { resolveHarnessLinks } from './harness-layout.mjs';
...
function assertExpectedLinks(home, { chainedSkills = false } = {}) {
  const expected = resolveHarnessLinks(REPO, home);
  ...
  for (const spec of expected) {
    ...
    const expectedRawTarget = chainedSkills && spec.id === 'shared-skills'
      ? '../.claude/skills'
      : spec.targetPath;
  ...
  assert.equal(lstatOrNull(join(home, '.grok', 'AGENTS.md')), null);
}

test('installer creates exactly the four discovery links in an empty home', (t) => {
  const home = temporaryDirectory(t, 'agent-config-home-');
  const result = runNode(INSTALLER, home);

  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /created 4 link\(s\), migrated 0/);
...
test('installer upgrades the current chained-link topology without rewriting it', (t) => {
  ...
  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /created 1 link\(s\), migrated 1/);
  assert.equal(readlinkSync(join(home, '.agents', 'skills')), '../.claude/skills');
  assertExpectedLinks(home, { chainedSkills: true });
});
