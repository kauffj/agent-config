...
test('installer creates exactly the five discovery links in an empty home', (t) => {
  const home = temporaryDirectory(t, 'agent-config-home-');
  const result = runNode(INSTALLER, home);

  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /created 5 link\(s\), migrated 0/);
...
test('installer migrates only the exact legacy Codex instruction link', (t) => {
  ...
  const result = runNode(INSTALLER, home);
  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /created 4 link\(s\), migrated 1/);
  assertExpectedLinks(home);
});
